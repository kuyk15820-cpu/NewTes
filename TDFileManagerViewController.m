...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    ...
    NSInteger count = 0;
    if (section == 0) {
        format = @"IPAs ที่ถอดรหัสแล้ว (%lu)";
        count = _decryptedIPAURLs.count;
    } else {
        format = @"ไฟล์ไบนารีที่ถอดรหัสแล้ว (%lu)";
...
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath {
    UIContextualAction *deleteAction = [UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive title:@"ลบ" handler:^(UIContextualAction *action, __kindof UIView *sourceView, void (^completionHandler)(BOOL)) {
