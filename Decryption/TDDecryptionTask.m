...
- (void)executeWithCompletionHandler:(void (^)(BOOL success, NSURL *outputURL, NSError *error))completionHandler
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        if (!options.decryptBinaryOnly) {    
            progress(@"กำลังคัดลอกชุดแอปพลิเคชัน...");
            ...
            for (NSString *item in frameworkContents) {
                if ([item hasSuffix:@".framework"]) {
                    ...
                    if ([self->_fileManager fileExistsAtPath:frameworkBinaryPath]) {
                        progress([NSString stringWithFormat:@"กำลังถอดรหัส framework %ld/%ld: %@...", (long)currentFrameworkIndex, (long)frameworkCount, frameworkName]);
        ...
        for (LSPlugInKitProxy *extension in self->_applicationProxy.plugInKitPlugins) {
            ...
            progress([NSString stringWithFormat:@"กำลังถอดรหัสส่วนขยาย %ld/%ld...",
        ...
        progress(@"กำลังสร้างไฟล์ IPA... (อาจใช้เวลาสักครู่)");
