...
- (void)_configureSearchController {
    ...
    _searchController.searchBar.placeholder = @"ค้นหาแอปพลิเคชัน";
...
- (void)_manualBundleIdentifierButtonTapped {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"ป้อนรหัสบันเดิลด้วยตนเอง" message:@"กรอกรหัสบันเดิลของแอปพลิเคชันที่คุณต้องการถอดรหัส" preferredStyle:UIAlertControllerStyleAlert];
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.placeholder = @"com.example.app";
    }];
    
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"ยกเลิก" style:UIAlertActionStyleCancel handler:nil];
    UIAlertAction *decryptAction = [UIAlertAction actionWithTitle:@"ถอดรหัส" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        if (!application.bundleURL) {
            UIAlertController *errorAlert = [UIAlertController alertControllerWithTitle:@"ข้อผิดพลาด" message:@"No application found with the specified bundle identifier." preferredStyle:UIAlertControllerStyleAlert];
            [errorAlert addAction:[UIAlertAction actionWithTitle:@"ตกลง" style:UIAlertActionStyleDefault handler:nil]];
...
- (void)loadAvailableApplications {
    ...
    // เปลี่ยนมาใช้ allApplications เพื่อดึงทุกอย่างที่ระบบรู้จัก
    NSArray <LSApplicationProxy *> *allInstalledApps = [[LSApplicationWorkspace defaultWorkspace] allApplications];
    
    [allInstalledApps enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(LSApplicationProxy *proxy, NSUInteger idx, BOOL *stop) {
        _LSApplicationState *state = proxy.appState;
        
        // 1. ตรวจสอบว่าเป็นแอปที่ติดตั้งสำเร็จและไม่ใช่ Placeholder
        if (!state.isInstalled || state.isPlaceholder) return;
        
        // 2. ตรวจสอบว่ามี Bundle URL (มีไฟล์อยู่ในเครื่อง)
        if (!proxy.bundleURL) return;

        // 3. กรองแอปของระบบออก (System Apps)
        // ปกติแอปที่ไม่ใช่ของระบบจะมีประเภทเป็น "User"
        if (![proxy.applicationType isEqualToString:@"User"]) {
            // กรณีพิเศษ: ตรวจสอบว่าเป็นแอปที่ติดตั้งผ่าน TrollStore หรือเครื่องมืออื่นที่อาจไม่อยู่ในกลุ่ม User 
            // แต่ไม่ใช่แอปที่อยู่ใน /Applications (System)
            if ([proxy.bundleURL.path hasPrefix:@"/Applications"]) return;
        }

        // 4. ตรวจสอบข้อมูลพื้นฐานเพื่อป้องกันการแครช
        if (![proxy atl_bundleIdentifier] || ![proxy atl_nameToDisplay]) return;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *title = @"ถอดรหัส";
    NSString *subtitle = [NSString stringWithFormat:@"คุณต้องการถอดรหัส %@ หรือไม่?", [application atl_nameToDisplay]];
    ...
    UIAlertAction *decryptBinaryOnlyAction = [UIAlertAction actionWithTitle:@"เฉพาะไฟล์ไบนารีหลัก" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
    ...
    UIAlertAction *decryptAction = [UIAlertAction actionWithTitle:@"IPA ทั้งหมด" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        [self _decryptApplicationProxy:application];
    }];
    [alert addAction:decryptAction];
...
- (void)_decryptApplicationProxy:(LSApplicationProxy *)application options:(TDDecryptionTaskOptions)options {
    ...
    decryptionTask.progressHandler = ^(NSString *message) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self->_progressAlert =
                [UIAlertController alertControllerWithTitle:@"กำลังถอดรหัส"
                                                    message:message
    ...
    [decryptionTask executeWithCompletionHandler:^(BOOL success, NSURL *outputURL, NSError *error) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (success) {
                resultAlert = [UIAlertController alertControllerWithTitle:@"สำเร็จ" message:@"ถอดรหัสเสร็จสิ้น!" preferredStyle:UIAlertControllerStyleAlert];
                
                // @iCrazeiOS
                if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"filza://"]]) {
                    [resultAlert addAction:[UIAlertAction actionWithTitle:@"แสดงใน Filza" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                        NSURL *url = [[NSURL URLWithString:@"filza://view"] URLByAppendingPathComponent:[outputURL path]];
                        [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:nil];
                    }]];
                }
            } else {
                resultAlert = [UIAlertController alertControllerWithTitle:@"ข้อผิดพลาด" message:error.localizedDescription preferredStyle:UIAlertControllerStyleAlert];
            }
            
            [resultAlert addAction:[UIAlertAction actionWithTitle:@"ตกลง" style:UIAlertActionStyleDefault handler:nil]];
